...
@section('content')
    <form action="/educations/{{ $education->id }}" method="post"enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <label for="">Education</label>
        <input type="text" name="name_edu" value="{{ old('name_edu', $education->name_edu) }}">

        <label> Start:</label>
        <input type="text" name="start_date" class="datepicker startDate" placeholder="MM-YYYY"
            value="{{ $education->sta_month }}-{{ $education->sta_year }}" value="{{ old('start_date') }}">
        <label> End:</label>
        <input type="text" name="end_date" class="datepicker endDate" placeholder="MM-YYYY"
            value="{{ $education->end_month }}-{{ $education->end_year }}" value="{{ old('end_date') }}">
        ...
        <div id="skillsContainer">
            ...
            @if (old('skills'))
                ...
                @endforeach
            @elseif($education->skills && $education->skills->count())
        ...
        <textarea type="text" name="description"
            value="{{ $education->description }}"style="width: 100%; height: 200px; font-size: 14px; padding: 10px;">
